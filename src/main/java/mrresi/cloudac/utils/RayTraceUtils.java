...
public class RayTraceUtils {
    ...
    private static final double BOX_EPSILON = 1e-4D;

    private static boolean isVecInYZ(SimpleCollisionBox box, Vector vec) {
        return vec != null && vec.getY() >= (box.minY - BOX_EPSILON) && vec.getY() <= (box.maxY + BOX_EPSILON)
            && vec.getZ() >= (box.minZ - BOX_EPSILON) && vec.getZ() <= (box.maxZ + BOX_EPSILON);
    }

    private static boolean isVecInXZ(SimpleCollisionBox box, Vector vec) {
        return vec != null && vec.getX() >= (box.minX - BOX_EPSILON) && vec.getX() <= (box.maxX + BOX_EPSILON)
            && vec.getZ() >= (box.minZ - BOX_EPSILON) && vec.getZ() <= (box.maxZ + BOX_EPSILON);
    }

    private static boolean isVecInXY(SimpleCollisionBox box, Vector vec) {
        return vec != null && vec.getX() >= (box.minX - BOX_EPSILON) && vec.getX() <= (box.maxX + BOX_EPSILON)
            && vec.getY() >= (box.minY - BOX_EPSILON) && vec.getY() <= (box.maxY + BOX_EPSILON);
    }
