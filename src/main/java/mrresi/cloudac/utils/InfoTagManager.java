...
public class InfoTagManager {
    ...
    private void startTask() {
        ...
        teleportTask = Bukkit.getScheduler().runTaskTimer(plugin, () -> {
            ...
            int currentTick = entityIdSeq.incrementAndGet();
            for (UUID viewerId : enabledViewers) {
                Player viewer = Bukkit.getPlayer(viewerId);
                if (viewer == null || !viewer.isOnline()) {
                    enabledViewers.remove(viewerId);
                ...
                for (Player target : Bukkit.getOnlinePlayers()) {
                    ...
                    if (hologram == null) {
                        ...
                        Location targetLoc = target.getLocation();
                        if (hologram.getLastLocation().getWorld() != null && !hologram.getLastLocation().getWorld().equals(targetLoc.getWorld())) {
                            destroyHologram(viewer, targetId);
                            spawnHologram(viewer, target);
                            continue;
                        }
                        if (hologram.getLastLocation().distanceSquared(targetLoc) >= (1.99 * 1.99)) {
