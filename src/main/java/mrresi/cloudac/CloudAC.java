...
import mrresi.cloudac.utils.InfoTagManager;
...
public final class CloudAC extends JavaPlugin {
    ...
    private InfoTagManager infoTagManager;
    ...
    public void onEnable() {
        ...
        infoTagManager = new InfoTagManager(this);
        ...
        getServer().getScheduler().runTaskTimerAsynchronously(this, () -> {
            File autoSaveFile = new File(getDataFolder(), "dataset.json");
            if (ai.hasExternalFileChanged(autoSaveFile)) {
                getLogger().info("[CloudAC] Detected external modification in dataset.json. Reloading dataset...");
                try {
                    int count = ai.loadDataset(autoSaveFile);
                    getLogger().info("[CloudAC] Successfully reloaded " + count + " samples from dataset.json!");
                } catch (IOException e) {
                    getLogger().warning("[CloudAC] Failed to reload modified dataset.json: " + e.getMessage());
                }
                return;
            }
            if (ai.isDatasetModifiedSinceSave() && ai.getDatasetSize() > 0) {
                try {
                    ai.saveDataset(autoSaveFile);
    ...
    public void onDisable() {
        if (infoTagManager != null) {
            infoTagManager.clearAll();
        }
    ...
    public InfoTagManager getInfoTagManager() {
        return infoTagManager;
    }
