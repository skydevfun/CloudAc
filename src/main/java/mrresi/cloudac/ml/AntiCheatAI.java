...
public class AntiCheatAI {
    ...
    public void saveDataset(File file) throws IOException {
        ...
        DatasetJson dj = new DatasetJson();
        dj.legit = new ArrayList<>();
        dj.soft = new ArrayList<>();
        for (TrainingData td : snapshot) {
    ...
    private String collapseJsonArrays(String json) {
        java.util.regex.Pattern pattern = java.util.regex.Pattern.compile("\\[\\s*([-0-9.eE+,\\s]+)\\s*\\]");
        java.util.regex.Matcher matcher = pattern.matcher(json);
        StringBuffer sb = new StringBuffer();
        while (matcher.find()) {
            String content = matcher.group(1);
            String collapsed = "[" + content.replaceAll("\\s+", "").replace(",", ", ") + "]";
            matcher.appendReplacement(sb, java.util.regex.Matcher.quoteReplacement(collapsed));
        }
            ...
            if (element.isJsonArray()) {
                ...
                for (com.google.gson.JsonElement item : array) {
                    if (item.isJsonObject()) {
                        ...
                        double label = obj.get("label").getAsDouble();
                        com.google.gson.JsonElement featElement = obj.has("legit_features") && !obj.get("legit_features").isJsonNull() ?
                                obj.get("legit_features") : obj.get("soft_features");
                        if (featElement != null && featElement.isJsonArray()) {
                            com.google.gson.JsonArray featArray = featElement.getAsJsonArray();
                            if (featArray.size() == TOTAL_FEATURES) {
                                double[] features = new double[TOTAL_FEATURES];
                                for (int i = 0; i < TOTAL_FEATURES; i++) {
                                    features[i] = featArray.get(i).getAsDouble();
                                }
                                loaded.add(new TrainingData(features, label));
                            }
                        }
                    }
                }
                ...
                if (obj.has("legit") && obj.get("legit").isJsonArray()) {
                    ...
                    for (com.google.gson.JsonElement item : legitArray) {
                        if (item.isJsonArray()) {
                            com.google.gson.JsonArray featArray = item.getAsJsonArray();
                            if (featArray.size() == TOTAL_FEATURES) {
                                double[] features = new double[TOTAL_FEATURES];
                                for (int i = 0; i < TOTAL_FEATURES; i++) {
                                    features[i] = featArray.get(i).getAsDouble();
                                }
                                loaded.add(new TrainingData(features, 0.0));
                            }
                        }
                    }
                }
                    ...
                    for (com.google.gson.JsonElement item : softArray) {
                        if (item.isJsonArray()) {
                            ...
                            if (featArray.size() == TOTAL_FEATURES) {
                                ...
                                loaded.add(new TrainingData(features, 1.0));
                            }
                        }
                    }
                }
    ...
    private static class DatasetJson {
        List<List<Double>> legit;
        List<List<Double>> soft;
    }
